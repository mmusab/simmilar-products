...
print(shape_img)
imgs_train = [np.array(x).reshape(500,500,1) for x in imgs_train]
imgs_test = [np.array(x).reshape(500,500,1) for x in imgs_test]
shape_img = imgs_train[0].shape
...
if modelName in ["simpleAE", "convAE"]:
    if trainModel:
        ...
        model.fit(X_train, n_epochs=n_epochs, batch_size=64)
        model.save_models()
    else:
...
for i, emb_flatten in enumerate(E_test_flatten):
    ...
    img_query = np.array(img_query).reshape(500,500) #/////////////////
    imgs_retrieval = [np.array(x).reshape(500,500) for x in imgs_retrieval] #/////////////////
