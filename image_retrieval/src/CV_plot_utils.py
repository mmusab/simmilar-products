...
def plot_query_retrieval(img_query, imgs_retrieval, outFile, title_names,que):
    ...
    ax.set_title("Query\n"+"web_id: "+que[0].split('_')[0]+"\n"+"product_id: "+que[0].split('_')[1].split('.')[0],  fontsize=14)

    # Plot retrieval images
    f = open("match_results.txt", "w")
    for i, img in enumerate(imgs_retrieval):
        ...
        ax.set_title("web_id: "+title_names[i].split('_')[0]+'\n'+"product_id: "+title_names[i].split('_')[1].split('.')[0], fontsize=8)  # set subplot title
        f.write("website_id: "+title_names[i].split('_')[0]+"    product_id: "+title_names[i].split('_')[1].split('.')[0]+'\n')
