...
test_name = os.listdir( dataTestDir )
...
knn = NearestNeighbors(n_neighbors=len(train_name), metric="cosine")
...
for i, emb_flatten in enumerate(E_test_flatten):
    ...
    for k in range(0, len(tmp)):
        tmp[k] = int(tmp[k]) 
    names = [train_name[ii] for ii in tmp]
    img_query = imgs_test[i] # query image
    imgs_retrieval = [imgs_train[idx] for idx in indices.flatten()] # retrieval images
    outFile = os.path.join(outDir, "{}_retrieval_{}.png".format(modelName, i))
    plot_query_retrieval(img_query, imgs_retrieval, outFile, names, test_name)
